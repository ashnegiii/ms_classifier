...
This repository consists of three deliverables:
...
This repository is the archive of all created resources.
...
- This will start the backend on port 8000: **http://localhost:8000**

**Model folder:** After using the download command from above, the four models (`.pth` files) are stored in the **`visualizer/models`** folder. 

In the frontend app, choose **Architecture** and click **Load model** — the server loads the matching file from `models/` automatically (or set `MODEL_DIR` in the environment file (`.env`) file).
...
The training pipeline is in `src/`. You run it from the **repository root** with the `src` directory on `PYTHONPATH` (e.g. `python src/train.py` or from inside `src/`). Make sure to first install the requirements! using:
`pip install -r requirements.txt`
