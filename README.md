...
This fetches the dataset into `data/raw` (which were used for the training/test pipeline) and also downloads the final trained **models** folder from Google Drive into `data/raw/models` (only if that folder doesn’t already exist). Do this before training or running the visualizer. The visualizer is used for showing the Predictions and GradCam implementation for future Lectures.
...
**Model folder:** After using the download command from above, the four models (`.pth` files) are stored in the `**data/raw/models`** folder. 
