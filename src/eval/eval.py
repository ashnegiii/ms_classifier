...
def main():
    ...
    test_metrics = test_step(model, test_dataloader, optimal_thresholds=optimal_thresholds,
                             class_names=class_names, loss_fn=loss_fn, device=device)

    print(f"  Test - Loss: {test_metrics['loss']:.4f}")
    print(
        f"  Test - Accuracy per class: {[f'{a:.4f}' for a in test_metrics['accuracy_per_class']]}")
    print(
        f"  Test - Precision per class: {[f'{a:.4f}' for a in test_metrics['precision_per_class']]}")
    print(
        f"  Test - Recall per class: {[f'{a:.4f}' for a in test_metrics['recall_per_class']]}")
    print(
        f"  Test - Average Precision per class: {[f'{a:.4f}' for a in test_metrics['ap_per_class']]}")
